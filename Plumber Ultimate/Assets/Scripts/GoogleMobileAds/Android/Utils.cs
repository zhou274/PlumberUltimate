...
namespace GoogleMobileAds.Android
{
	internal class Utils
	{
		...
		public static AndroidJavaObject GetAdRequestJavaObject(AdRequest request)
		{
			...
			{
				androidJavaObject.Call<AndroidJavaObject>("addKeyword", new object[1]
				{
					keyword
				});
			}
			foreach (string testDevice in request.TestDevices)
			{
				if (testDevice == "SIMULATOR")
				{
					...
					androidJavaObject.Call<AndroidJavaObject>("addTestDevice", new object[1]
					{
						@static
					});
				}
			...
			if (request.Birthday.HasValue)
			{
				...
				androidJavaObject.Call<AndroidJavaObject>("setBirthday", new object[1]
				{
					androidJavaObject2
				});
			}
			if (request.Gender.HasValue)
			{
				...
				if (num.HasValue)
				{
					androidJavaObject.Call<AndroidJavaObject>("setGender", new object[1]
					{
						num
					});
				}
			...
			{
				androidJavaObject.Call<AndroidJavaObject>("tagForChildDirectedTreatment", new object[1]
				{
					request.TagForChildDirectedTreatment.GetValueOrDefault()
				});
			}
			androidJavaObject.Call<AndroidJavaObject>("setRequestAgent", new object[1]
			{
				"unity-3.13.1"
			});
			...
			androidJavaObject.Call<AndroidJavaObject>("addNetworkExtras", new object[1]
			{
				androidJavaObject4
			});
			foreach (MediationExtras mediationExtra in request.MediationExtras)
			{
				...
				{
					androidJavaObject6.Call<string>("put", new object[2]
					{
						extra2.Key,
						extra2.Value
					});
				}
				AndroidJavaObject androidJavaObject7 = androidJavaObject5.Call<AndroidJavaObject>("buildExtras", new object[1]
				{
					androidJavaObject6
				});
